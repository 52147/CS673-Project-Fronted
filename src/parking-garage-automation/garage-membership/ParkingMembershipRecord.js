...
const ParkingMembershipRecord = () => {
    ...
    return (<>
        ...
        {!loading&&<>
            <div className="row mt-5">
                ...
                <div className={`col-2 ${styles.textLeft}`}>
                    <Dropdown>
                        ...
                        <Dropdown.Menu>
                            <Dropdown.Item href="#Car" onClick={plateClickHandler}>plate</Dropdown.Item>
                            <Dropdown.Item href="#Motorcycle"
                                           onClick={usernameClickHandler}>username</Dropdown.Item>
                        </Dropdown.Menu>
                    </Dropdown>
                </div>
                ...
                <div className={`col-2 ${styles.textLeft}`}>
                    {
                        ...
                        <input value={username} onChange={(event) => setUsername(event.target.value)}
                            className="form-control" type="username" placeholder="search by username"/>
                    }
                    {
                        findWay==='plate'&&
                        <input value={plate} onChange={(event) => setPlate(event.target.value)}
                               className="form-control" type="username" placeholder="search by plate"/>
                    }
                </div>
            ...
            {
                historyAll.length === 0 && <h3 className={`mt-5 text-white`}>There is no such data. Please try again.</h3>
            }
            {
                historyAll.length !== 0 &&
